...
<!-- we will be using a click event modifier so that the event will only fire on a certain condition, in this case when we click on the background-->
  <div class="backdrop" @click.self="closeModal">
      ...
      <!-- slots is used to pass in full templates eg.Forms,Cards etc. -->
          <slot></slot>
          <!-- lets first create a div to contain our named slot -->
        <div class="namedSlot">
      <!--when passing in the naemd slot we need to give it a attribute of name with name we gave it in the template we ref (v-slot:"whatever name we placed here")   -->
